...
class GraphSelfFusion(nn.Module):
    def __init__(self, args):
        ...
        self.num_trans_layers = args.num_trans_layers
        ...
        for i in range(self.num_trans_layers-1):
    ...
    def reset_parameters(self):
        ...
        # for encoder in self.fusion_transformer_layers:
        #     encoder.reset_parameters()
        # Hard coding
    ...
    def forward(self, data):
        ...
        x_gcn_, x_trans_, x_mix = self.fusion_transformer_encoder1(batch_gcn, batch_trans, batch_mix)
        for encoder in self.fusion_transformer_layers:
            x_gcn_, x_trans_, x_mix = encoder(x_gcn_, x_trans_, x_mix)

        x_gcn_ = x_gcn_[mask_gcn]
        x_gcn = (x_gcn + x_gcn_) / 2
        x_trans_ = x_trans_[mask_trans]
        x_trans = (x_trans + x_trans_) / 2
        ...
        #     batch_gcn, mask_gcn = to_dense_batch(x_gcn, batch)
        #     batch_trans, mask_trans = to_dense_batch(x_trans, batch)
        #     # maintain gcn, trans representations now
        #     x_gcn_, x_trans_, x_mix = self.fusion_transformer_layers[i](batch_gcn, batch_trans, batch_mix)
        #     x_gcn_ = x_gcn_[mask_gcn]
        #     x_trans_ = x_trans_[mask_trans]
        #     x_mix = x_mix[mask_mix]
        #     # fusion2: mix-up
...
class GraphSelfFusionTransMix(nn.Module):
    def __init__(self, args):
        super(GraphSelfFusionTransMix, self).__init__()
        self.args = args
        #* init parameters
        self.num_mix_layers = args.num_fusion_layers
        # gcn layer parameters
        self.fusion_type    = args.fusion_type
        self.gcn_channels   = args.gcn_channels
        self.num_features   = args.in_size
        ...
        self.encoders = torch.nn.ModuleList()
        self.fusion_transformer_layers = nn.ModuleList()
        for i in range(self.num_mix_layers-1):
            alpha_1 = np.random.normal(loc=self.alpha, scale=self.alpha)
            if alpha_1 >= 1:
                alpha_1 = self.alpha
            self.encoders.append(SelfMultiFusionLayer(self.args, position="m"))
...
class GraphSelfFusionMix(nn.Module):
    def __init__(self, args):
        super(GraphSelfFusionMix, self).__init__()
        self.args = args
        #* init parameters
        self.num_layers = args.num_fusion_layers
        # gcn layer parameters
